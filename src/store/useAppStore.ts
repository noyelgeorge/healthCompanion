            ...
            syncWithFirestore: async () => {
                ...
                try {
                    ...
                    if (userDoc.exists()) {
                        ...
                        if (userDoc.data()?.streaks) {
                            set({ streaks: userDoc.data().streaks })
                        }
                    }
                    ...
                    // Sync medicine data from Firebase (new medicine Firebase integration)
                    try {
                        const { useMedicineStore } = await import('../medicine/store/medicineStore')
                        await useMedicineStore.getState().syncMedicineWithFirebase(state.user.uid!)
                    } catch (err) {
                        console.error('[AppStore] Medicine sync failed:', err)
                    }
                    ...
                    const unsubProfile = onSnapshot(doc(db, 'users', state.user.uid!), (snap) => {
                        if (snap.exists()) {
                            ...
                            if (snap.data()?.streaks) {
                                set({ streaks: snap.data().streaks })
                            }
                        }
                    })
            ...
            wipeAllData: async () => {
                ...
                if (isAuthenticated && uid) {
                    try {
                        ...
                        // NEW: Also wipe medicine subcollection documents
                        const medicineDocs = ['profile', 'medicines', 'takenToday', 'adherenceHistory', 'badges']
                        await Promise.all(
                            medicineDocs.map(docName =>
                                deleteDoc(doc(db, 'users', uid, 'medicine', docName))
                                    .catch(() => { }) // ignore if doc doesn't exist
                            )
                        )

                        // NEW: Clean up medicine Firebase listener
                        try {
                            const { useMedicineStore } = await import('../medicine/store/medicineStore')
                            useMedicineStore.getState().cleanupMedicineListener()
                        } catch { /* ignore */ }
