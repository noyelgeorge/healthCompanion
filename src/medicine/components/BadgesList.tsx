...
export const BadgesList = ({ badges }: BadgesListProps) => {
    const markBadgeSeen = useMedicineStore(state => state.markBadgeSeen)

    useEffect(() => {
        const timer = setTimeout(() => {
            badges.forEach(badge => {
                if (badge.unlockedAt && !badge.seenAt) {
                    markBadgeSeen(badge.id)
                }
            })
        }, 3000)
        return () => clearTimeout(timer)
    }, [badges, markBadgeSeen])
            ...
            <div className="grid grid-cols-2 gap-4">
                {badges.map((badge) => (
                    ...
                    >
                        ...
                        {badge.unlockedAt && (
                            <div className={cn(
                                "absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 bg-emerald-500 text-[8px] text-white font-black rounded-full uppercase tracking-tighter shadow-sm",
                                !badge.seenAt && "animate-bounce"
                            )}>
                                New
                            </div>
                        )}
