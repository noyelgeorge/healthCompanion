...
import { Check, Edit2, AlertCircle, Plus, Trash2, Loader2, ChevronDown, AlertTriangle, Clipboard } from 'lucide-react'
...
export const ScanReviewTab = ({ results, onConfirm, onDiscard }: ScanReviewTabProps) => {
    ...
    // Add state for manual time/stock overrides per card
    const [userOverrides, setUserOverrides] = useState<Record<number, { time: string; totalPills: string }>>({})

    const getOverride = (idx: number) => userOverrides[idx] ?? {
        time: localResults[idx]?.time ?? '09:00',
        totalPills: String(localResults[idx]?.quantity ?? '')
    }
    ...
    const handleConfirmCard = (idx: number) => {
        let med = localResults[idx]
        if (med.timeExplicit === false) {
            const override = getOverride(idx)
            med = {
                ...med,
                time: override.time || med.time,
                quantity: override.totalPills ? parseInt(override.totalPills, 10) : med.quantity,
            }
        }
    ...
    return (
        <div className="space-y-4">
            ...
            {localResults.length > 0 && (
                <div className="flex flex-wrap justify-between items-center gap-2 mb-2 px-1">
            ...
            {localResults.length === 0 && (
                <div className="glass p-10 rounded-[2.5rem] flex flex-col items-center justify-center text-center space-y-5">
                    <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-950/30 flex items-center justify-center text-indigo-400">
                        <Clipboard size={24} />
                    </div>
                    <div>
                        <h3 className="font-black text-slate-700 dark:text-white text-sm">No pending scans</h3>
                        <p className="text-xs text-slate-400 mt-1 flex flex-col items-center">
                            <span>Upload a prescription or add a medicine manually</span>
                        </p>
                    </div>
                    <button
                        onClick={() => setShowManualForm(true)}
                        className="w-full bg-indigo-500 text-white rounded-2xl py-3 font-black uppercase tracking-widest text-[11px] shadow-lg shadow-indigo-500/20 hover:bg-indigo-600 transition-all active:scale-95"
                    >
                        Add Manually
                    </button>
                </div>
                ...
                {localResults.map((med, idx) => {
                    ...
                    return (
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="p-2.5 min-w-[40px] min-h-[40px] flex items-center justify-center text-slate-400 hover:text-indigo-500 transition-colors"
                                    >
                                        <Edit2 size={16} />
                                    </button>
                                    <button
                                        onClick={() => onDiscard(idx)}
                                        className="p-2.5 min-w-[40px] min-h-[40px] flex items-center justify-center text-slate-400 hover:text-rose-500 transition-colors"
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            ...
                            {/* Only shown when doctor did not specify time */}
                            {med.timeExplicit === false && (
                                <div className="mb-4 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 space-y-3">
                                    <div className="flex items-center gap-2 mb-1">
                                        <AlertTriangle size={13} className="text-amber-500" />
                                        <p className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest">
                                            Doctor did not specify a time — please set manually
                                        </p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input
                                                type="time"
                                                value={getOverride(idx).time}
                                                onChange={e => setUserOverrides(prev => ({
                                                    ...prev,
                                                    [idx]: { ...getOverride(idx), time: e.target.value }
                                                }))}
                                                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-800 rounded-xl text-sm font-bold text-slate-800 dark:text-white focus:outline-none focus:border-indigo-400"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                                            ...
                                            <input
                                                ...
                                                min={0}
                                                placeholder="e.g. 30"
                                                value={getOverride(idx).totalPills}
                                                onChange={e => setUserOverrides(prev => ({
                                                    ...prev,
                                                    [idx]: { ...getOverride(idx), totalPills: e.target.value }
                                                }))}
                                                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-800 rounded-xl text-sm font-bold text-slate-800 dark:text-white placeholder:text-slate-300 focus:outline-none focus:border-indigo-400"
                                            />
                                        </div>
                                    </div>
                                </div>
