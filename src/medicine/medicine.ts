import { getApiKey, GEMINI_API_URL, fetchWithRetry } from "../lib/ai";
...
export interface ScannedMedicine {
    ...
    timeExplicit?: boolean; // NEW: true = doctor specified time, false = AI guessed
}
...
export async function analyzeMedicineImage(file: File): Promise<ScannedMedicine[]> {
                ...
                parts: [
                    {
                        ...
                        text: `ACT AS A PHARMACIST. 
                                ...
                                6. timeExplicit: true if the prescription explicitly states WHEN to take (e.g., "morning", "8 AM", "after dinner"). false if you are only guessing from general practice. This is critical.
    ...
    if (!response.ok) {
        ...
        throw new Error(errData?.error?.message || "AI analysis failed. Please check your API key in Profile.");
    }
    ...
    try {
        ...
        return (data || []).map((m: Record<string, unknown>) => ({
            ...
            timeExplicit: typeof m.timeExplicit === 'boolean' ? m.timeExplicit : true,
        }));
    } catch (parseError) {
...
Return ONLY a JSON object:
{
  ...
  "sideEffects": "Most common side effects in 1-2 sentences"
    ...
    if (!response.ok) {
        const errData = await response.json().catch(() => ({}));
        throw new Error(errData?.error?.message || "AI overview fetch failed");
    }
    const result = await response.json();
