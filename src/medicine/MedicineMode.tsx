...
export default function MedicineMode() {
    ...
    const onConfirmScan = (med: ScannedMedicine) => {
        ...
        const totalPills = med.quantity ?? 0
        // Distribute pills among doses to maintain correct stock tracking
        // (e.g., 30 pills taken 3x a day = 10 pills tracked per scheduled slot)
        const pillsPerDose = Math.floor(totalPills / freq)

        for (let i = 0; i < freq; i++) {
            let scheduledTime = med.time
            if (i > 0) {
                // Stagger doses across a 16-hour waking window
                const [h, m] = med.time.split(':').map(Number)
                const hoursPerDose = Math.max(4, Math.floor(16 / freq))
                const newH = (h + (hoursPerDose * i)) % 24
                scheduledTime = `${String(newH).padStart(2, '0')}:${String(m).padStart(2, '0')}`
            }
        ...
        toast.success(`${med.name} added for ${selectedFamilyMember}! ${freq > 1 ? `(${freq} daily doses created)` : ''}`)
    }
    ...
    return (
        <div className="page-container bg-slate-50 dark:bg-slate-950 pb-32 min-h-screen">
            ...
            <div className="sticky top-0 z-30 flex items-center justify-between p-4 glass dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        disabled={isScanning}
                        className="px-4 py-2 h-10 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center gap-1.5 hover:border-indigo-400 transition-all disabled:opacity-50"
                    >
                        {isScanning ? <Loader2 size={14} className="animate-spin text-indigo-500" /> : <Upload size={14} />}
                        Upload
                    </button>
                    <button
            ...
            <div className="max-w-2xl mx-auto p-4 space-y-8">
                {/* Navigation Tabs */}
                <div className="flex p-1.5 bg-slate-100 dark:bg-slate-900 rounded-xl gap-1 overflow-x-auto scrollbar-hide pb-1 -mx-1 px-1">
                    <button
                        onClick={() => setActiveTab('basket')}
                        className={cn(
                            "flex-shrink-0 flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-black uppercase tracking-widest text-[10px] transition-all",
                            activeTab === 'basket'
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-shrink-0 flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-black uppercase tracking-widest text-[10px] transition-all relative",
                            activeTab === 'review'
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between px-1">
                                    ...
                                    <button
                                        ...
                                        className="text-[10px] font-black text-indigo-500 uppercase tracking-widest hover:underline flex flex-wrap items-center justify-center gap-1 p-2.5 min-w-[40px] min-h-[40px]"
                                    >
                                                ...
                                                <div className="grid gap-4">
                                                    {meds.map((med) => (
                                                        <MedicineCard
                                                            ...
                                                            onUpdateSchedule={(newTime) => {
                                                                updateMedicine(med.id, { schedule: newTime })
                                                            }}
                                                        />
                                                    ))}
                                                </div>
