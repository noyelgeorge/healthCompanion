...
import { Plus, Trash2, Loader2, Camera, X, Pencil, Search, ImageIcon } from "lucide-react"
        ...
        <div className="page-container space-y-6">
            ...
            <div className="glass-dark p-6 rounded-3xl text-white overflow-hidden relative">
                ...
                <div className="mt-4 pt-4 border-t border-slate-800/50">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1 mb-1">Today total</p>
                ...
                {TABS.map(tab => (
                    ...
                    >
                        <div className="flex items-center gap-1.5">
                            <span>{tab}</span>
                            {(() => {
                                const count = (logs?.entries || []).filter(e => e.mealType === tab).length;
                                return count > 0 ? (
                                    <span className="w-4 h-4 rounded-full bg-orange-500 text-white text-[8px] font-black flex items-center justify-center">{count}</span>
                                ) : null;
                            })()}
                        </div>
                        {(() => {
                                        ...
                                        <div key={entry.id} className="glass p-4 rounded-3xl flex justify-between items-center hover:scale-[1.01] transition-transform group">
                                            <div className="flex items-center gap-4">
                                                <div className="relative">
                                                    ...
                                                    {entry.healthScore && (
                                                        <div
                                                            title={`Health score: ${entry.healthScore}/10 (AI-rated)`}
                                                            className={cn(
