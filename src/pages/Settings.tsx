...
import { ArrowLeft, Key, Download, Activity, ShieldCheck, ExternalLink } from "lucide-react"
...
export default function Settings() {
    ...
    const [draftUsdaKey, setDraftUsdaKey] = useState("")
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center px-1">
                            ...
                            {apiKey && !isEditingGeminiKey && (
                                <button onClick={() => { setDraftGeminiKey(""); setIsEditingGeminiKey(true); }} className="text-[9px] font-black text-indigo-500 uppercase tracking-widest hover:underline">Change Key</button>
                            )}
                        </div>
                            ...
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    <input
                                        ...
                                        className={cn(
                                            "w-full p-4 bg-white/50 dark:bg-slate-950/50 border rounded-2xl focus:outline-none transition-all font-bold text-xs",
                                        ...
                                        onChange={(e) => {
                                            setDraftGeminiKey(e.target.value)
                                            setKeyStatus('idle')
                                        }}
                                    />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <button
                                        onClick={() => { setApiKey(draftGeminiKey); setIsEditingGeminiKey(false); }}
                                        className="flex-1 px-4 flex items-center justify-center bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all"
                                    >
                                        Save
                                    </button>
                                    {apiKey && (
                                        <button
                                            ...
                                            className="flex-1 px-4 flex items-center justify-center bg-slate-200 dark:bg-slate-800 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all"
                                        >
                                            Cancel
                                        </button>
                                    )}
                            ...
                            <div className="p-4 bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 font-sans tracking-normal">
            ...
            <div className="space-y-3 pt-6 border-t border-slate-100 dark:border-slate-800/50">
                <div className="flex justify-between items-center px-1">
                    ...
                    {usdaApiKey && !isEditingUsdaKey && (
                        <button onClick={() => { setDraftUsdaKey(""); setIsEditingUsdaKey(true); }} className="text-[9px] font-black text-emerald-500 uppercase tracking-widest hover:underline">Change Key</button>
                    )}
                </div>
                ...
                {(!usdaApiKey || isEditingUsdaKey) ? (
                    <div className="flex gap-2">
                        <div className="relative flex-1">
                            <input
                                ...
                                className="w-full p-4 bg-white/50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 rounded-2xl focus:outline-none transition-all font-bold text-xs focus:ring-emerald-500/30"
                                value={draftUsdaKey}
                        ...
                        <div className="flex flex-col gap-1">
                            <button
                                onClick={() => { setUsdaApiKey(draftUsdaKey); setIsEditingUsdaKey(false); }}
                                className="flex-1 px-4 flex items-center justify-center bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all"
                            >
                                Save
                            </button>
                    ...
                    <div className="p-4 bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 font-sans tracking-normal">
                                    {'•'.repeat(Math.max(0, usdaApiKey.length - 4)) + usdaApiKey.slice(-4)}
                                </span>
                                <span className="text-[8px] font-bold text-emerald-600/60 dark:text-emerald-400/60 uppercase tracking-widest">Key Saved Locally</span>
                            </div>
                        </div>
