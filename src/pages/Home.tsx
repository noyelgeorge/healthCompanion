...
export default function Home() {
    ...
    return (
        <div className="page-container bg-gradient-to-br from-slate-50 via-white to-orange-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
            ...
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-orange-200/10 dark:bg-orange-500/5 rounded-full blur-[100px] -z-10"></div>
                ...
                <Link to="/stats" className="block hover:scale-[1.01] transition-transform active:scale-95 group">
                    <div className="px-3 py-2 rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between gap-2 group-hover:border-orange-500/30 transition-colors">
                        <div className="flex w-full gap-2">
                            {/* Calories Chip */}
                            <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700/50">
                                <div className="flex items-center gap-1.5 mb-1.5">
                                    <div className="w-1.5 h-1.5 rounded-full bg-orange-500"></div>
                                    <span className="text-[9px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest">{totalCalories} / {calorieTarget} kcal</span>
                                </div>
                                <div className="h-1 w-full bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                                    <div className="h-full bg-orange-500 rounded-full" style={{ width: `${Math.min(100, calorieTarget > 0 ? (totalCalories / calorieTarget) * 100 : 0)}%` }}></div>
                                </div>
                            </div>
                            {/* Water Chip */}
                            <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700/50">
                                <div className="flex items-center gap-1.5 mb-1.5">
                                    ...
                                    <span className="text-[9px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest">{waterIntake} / {user.waterGoal} ml</span>
                                </div>
                                <div className="h-1 w-full bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min(100, user.waterGoal > 0 ? (waterIntake / user.waterGoal) * 100 : 0)}%` }}></div>
                                </div>
                            </div>
                            {/* Streak Chip */}
                            <div className="flex-1 flex flex-col justify-center bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border border-slate-100 dark:border-slate-700/50">
                                <div className="flex items-center gap-1.5">
                                    <div className="w-1.5 h-1.5 rounded-full bg-amber-500"></div>
                                    <span className="text-[9px] font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest">{streaks.current} day streak</span>
                                </div>
                            </div>
                        </div>
                        <ArrowRight size={14} className="text-slate-400 group-hover:text-orange-500 transition-colors shrink-0 ml-1" />
                    </div>
                </Link>
                        ...
                        <div className="mt-4 space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={cn(
                                        "flex-1 py-2 bg-white/10 rounded-xl backdrop-blur-md hover:bg-white/20 transition-all active:scale-95 flex items-center justify-center border border-white/10",
                                        waterIntake === 0 && "opacity-40 pointer-events-none"
                                    )}
                                >
                                    <Minus size={14} />
                                </button>
                    ...
                    <div className={cn(
                        ...
                        <div className="absolute bottom-0 right-6 flex flex-col items-end gap-4">
                            ...
                            <div className={cn(
                                ...
                                >
                                    <span className="px-3 py-1.5 glass dark:glass-dark rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 shadow-xl opacity-100">
                                        Post Recipe
                                    </span>
