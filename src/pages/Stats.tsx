        ...
        <div className="page-container space-y-6 bg-transparent text-slate-900 dark:text-slate-100 transition-colors duration-300">
            ...
            <div className="flex justify-between items-center gap-2 px-1">
                ...
                <Button
                    ...
                    onClick={handleShare}
                    disabled={isSharing}
                    className="h-10 px-4 rounded-2xl shadow-lg bg-gradient-to-r from-orange-500 to-rose-500 font-black uppercase tracking-widest text-[9px] py-0 shrink-0 disabled:opacity-50 flex items-center justify-center transition-all"
                >
                    {isSharing ? (
                        <div className="flex items-center gap-1.5"><Loader2 size={14} className="animate-spin text-white" /></div>
                    ) : (
                        <div className="flex items-center gap-1.5"><Share2 size={12} /> Share</div>
                    )}
                </Button>
            </div>
            ...
            <div className="relative group">
                ...
                <div className="relative glass-dark dark:glass-dark rounded-[2rem] p-6 overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        {isLoadingAI ? (
                            <div className="glass p-6 rounded-[2.5rem] space-y-3">
                                <div className="w-3/4 h-4 rounded-full bg-slate-200 dark:bg-slate-700 animate-pulse"></div>
                                <div className="w-1/2 h-4 rounded-full bg-slate-200 dark:bg-slate-700 animate-pulse"></div>
                                <div className="w-2/3 h-4 rounded-full bg-slate-200 dark:bg-slate-700 animate-pulse"></div>
                            </div>
                ...
                <div className="flex justify-between items-center mb-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {period === 'last30' && (
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">(30-day view — bars may appear smaller)</p>
                            )}
                        </div>
                    </div>
                    <div className="text-right">
            ...
            {weightHistory.length === 0 ? (
                <div className="glass dark:glass-dark rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800/50 flex flex-col items-center justify-center text-center space-y-3">
                    <Scale size={32} className="text-slate-300 dark:text-slate-600 mb-2" />
                    <h3 className="font-black text-slate-900 dark:text-white text-lg">No weight data yet</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest max-w-[200px]">Log your weight in Profile to start tracking your trend</p>
                    <a href="/profile" className="mt-2 text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-4 py-2 rounded-xl transition-colors">Go to Profile</a>
                </div>
            ) : weightHistory.length === 1 ? (
                <div className="glass dark:glass-dark rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800/50 flex flex-col items-center justify-center text-center space-y-3">
                    <Scale size={32} className="text-slate-300 dark:text-slate-600 mb-2" />
                    <h3 className="font-black text-slate-900 dark:text-white text-lg">Log one more weight entry</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest max-w-[200px]">Log your weight in Profile to start tracking your trend</p>
                    <a href="/profile" className="mt-2 text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-4 py-2 rounded-xl transition-colors">Go to Profile</a>
                </div>
            ) : (
