...
export default function Plans() {
    ...
    const allIngredients = (() => {
        const ingredients = (plans?.entries || []).flatMap(e => e.ingredients || [])
        const aggregated: Record<string, { quantity: number; unit: string }> = {}
        const unitless: Record<string, boolean> = {}

        ingredients.forEach(rawIng => {
            if (!rawIng) return
            // Regex to match "Quantity Unit Name" or "Quantity Name"
            // Examples: "500g Chicken", "2 cups Rice", "3 Eggs", "Salt"
            ...
            if (match) {
                ...
                unitless[rawIng] = true
            }
        })

        const result = Object.entries(aggregated).map(([key, data]) => {
            const [name] = key.split('|')
            const qty = data.quantity % 1 === 0 ? data.quantity : data.quantity.toFixed(1)
            // Capitalize name
            const capitalized = name.charAt(0).toUpperCase() + name.slice(1)
            return `${qty}${data.unit ? data.unit + ' ' : ' '}${capitalized}`.trim()
        })

        return [...result, ...Object.keys(unitless)]
    })()
    ...
    const handleShareWhatsApp = () => {
        if (checkedItems.length === 0) {
            toast.error("No items selected", {
                description: "Please mark the items you want to share first."
            })
            return
        }

        const dateStr = format(new Date(selectedDate), 'EEEE, MMM do')
        const items = checkedItems.map(ing => `- [x] ${ing}`).join('\n')
        const message = `🛒 My Grocery List for ${dateStr}\n\n${items}\n\nGenerated by Health Companion AI`
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        {checkedItems.length > 0 && (
                                            <button
                                                ...
                                                className="text-rose-500 hover:text-rose-400 font-black uppercase tracking-widest text-[10px] mt-1 transition-colors"
                                            >
                                                Clear checked
                                            </button>
                                        )}
